...
class ArtiqObjectProperties():
    ...
    def svgdrawobject(self, stl, dwg, dwgg, endofclass=False):
        stl.idx = stl.idx + 1
        dwgg.add(dwg.text(self.name, insert=(stl.column*stl.length*4 + 10, stl.idx*stl.height), fill=svgwrite.rgb(0, 128, 128, 'RGB')))
        for inherititems in self.inherit:
            stl.idx = stl.idx + 1
            dwgg.add(dwg.text(inherititems, insert=(stl.column*stl.length*4 + 10, stl.idx*stl.height), fill=svgwrite.rgb(128, 128, 128, 'RGB')))
        for contentitems in self.content:
            stl.idx = stl.idx + 1
            dwgg.add(dwg.text(contentitems, insert=(stl.column*stl.length*4 + 10, stl.idx*stl.height)))
        if not endofclass:
            dwgg.add(dwg.line((stl.column*stl.length*4 + 0, stl.idx*stl.height + 4), (stl.column*stl.length*4 + stl.length, stl.idx*stl.height + 4), stroke=svgwrite.rgb(0, 0, 0, '%')))
...
def main():
    ...
    for idx, fl in enumerate(filelines, start = filelinesidx):
                ...
                for inh in inherit:
                    ...
                    for cl in classes:
                        if cl.name == inh.strip():
                            classes[-1].inheritance.inherit.extend(cl.inheritance.inherit)
                            ...
                            classes[-1].specials.inherit.extend(cl.specials.inherit)
                            classes[-1].inheritance.inherit.extend(cl.inheritance.content)
                            ...
                            classes[-1].specials.inherit.extend(cl.specials.content)
                            break
        ...
        if -1 != fl.find("self.specials +=") or specialslineunfinished:
            ...
            if -1 != instancestarts:
                ...
                specialsname = fl[instancestarts:instancestarts+fl[instancestarts:].find("\"")]
    ...
    for cl in classes:
        ...
        clg.add(dwg.text(".".join(cl.hierarchy)+"."+cl.name, insert=(stl.column*stl.length*4 + 10, stl.idx*stl.height), fill=svgwrite.rgb(0, 64, 64, 'RGB')))
        clg.add(dwg.line((stl.column*stl.length*4 + 0, stl.idx*stl.height + 4), (stl.column*stl.length*4 + stl.length, stl.idx*stl.height + 4), stroke=svgwrite.rgb(0, 0, 0, 'RGB')))
        ...
        clg.add(dwg.rect(insert=(stl.column*stl.length*4 + 0,stl.rectstartidx*stl.height),size=(stl.length,stl.height*(stl.idx - stl.rectstartidx + 1)),fill="none", stroke=svgwrite.rgb(0, 0, 0, 'RGB'), stroke_width=1))
