...
rootpath = "YOUR ARTIQ PATH\\artiq_master\\artiq\\"
...
class ArtiqObjectProperties():
    ...
    def svgdrawobject(self, stl, dwg, dwgg, endofclass=False):
        stl.idx = stl.idx + 1
        dwgg.add(dwg.text(self.name, insert=(stl.column*stl.length*4 + 10,
                                             stl.idx*stl.height),
                          fill=svgwrite.rgb(0, 128, 128, 'RGB')))
        for inherititems in self.inherit:
            stl.idx = stl.idx + 1
            dwgg.add(dwg.text(inherititems,
                              insert=(stl.column*stl.length*4 + 10,
                                      stl.idx*stl.height),
                              fill=svgwrite.rgb(128, 128, 128, 'RGB')))
        for contentitems in self.content:
            stl.idx = stl.idx + 1
            dwgg.add(dwg.text(contentitems,
                              insert=(stl.column*stl.length*4 + 10,
                                      stl.idx*stl.height)))
        if not endofclass:
            dwgg.add(dwg.line((stl.column*stl.length*4 + 0,
                               stl.idx*stl.height + 4),
                              (stl.column*stl.length*4 + stl.length,
                               stl.idx*stl.height + 4),
...
class ArtiqClass():
    def __init__(self, classname, objectlist):
        ...
        self.specials = ArtiqObjectProperties("specials")
    ...
    def printclass(self):
        ...
        print(self.specials.name+": "+str(self.specials.content))


def scancodesignature(stringline, lrequiredsubstring,
                      rrequiredsubstring, lstripsubstring):
    lrequiredend = 1
    ...
    if -1 != lrequiredend and -1 != rrequiredend:
        ...
        if -1 != lstripindex:
            artiqname = stringline[lstripindex + len(lstripsubstring):
                                   rrequiredend]
        else:
            artiqname = stringline[len(stringline) - len(stringline.lstrip()):
                                   rrequiredend]
    return artiqname
...
class ImportFileObject():
    def __init__(self, fileline):
        importfromstr = fileline[fileline.find("from artiq.gateware")+20:
                                 fileline.find(" import")].strip()
        importlibstr = fileline[fileline.find(" import")+8:].strip()
        self.objectlist = []
        if importfromstr == "":
            importfilepath = rootpath + importlibstr
        else:
            importfilepath = (rootpath +
                              importfromstr.replace(".", "\\") + "\\" +
                              importlibstr)
...
def main():
    ...
    for idx, fl in enumerate(filelines, start=filelinesidx):
        if fl.startswith("from artiq.gateware"):
            ...
            if fl.endswith("import *\n"):
                ...
                filelines.insert(idx, "endofobject " +
                                      ".".join(ifo.objectlist) +
                                      "\n")
                idx = idx + 1
                filelines.insert(idx, "startofobject " +
                                      ".".join(currentobjectlist) +
                                      "\n")
                idx = idx + 1
                ...
                while 1:
                    ...
                    if filelines[idx - 1].endswith(")\n"):
                        objtoimport = fl[fl.find("(") + 1:
                                         fl.find(")")].split(",")
                        importedfilelines = []
                        for objti in objtoimport:
                            importedfilelines.append("from artiq.gateware." +
                                                     importpath +
                                                     " import " +
                                                     objti.strip() +
                                                     "\n")
                ...
                for inh in inherit:
                    ...
                    for cl in classes:
                        if cl.name == inh.strip():
                            classes[-1].inheritance.inherit.extend(
                                cl.inheritance.inherit)
                            classes[-1].submodules.inherit.extend(
                                cl.submodules.inherit)
                            classes[-1].signals.inherit.extend(
                                cl.signals.inherit)
                            classes[-1].clockdomains.inherit.extend(
                                cl.clockdomains.inherit)
                            classes[-1].objects.inherit.extend(
                                cl.objects.inherit)
                            classes[-1].functions.inherit.extend(
                                cl.functions.inherit)
                            classes[-1].specials.inherit.extend(
                                cl.specials.inherit)
                            classes[-1].inheritance.inherit.extend(
                                cl.inheritance.content)
                            classes[-1].submodules.inherit.extend(
                                cl.submodules.content)
                            classes[-1].signals.inherit.extend(
                                cl.signals.content)
                            classes[-1].clockdomains.inherit.extend(
                                cl.clockdomains.content)
                            classes[-1].objects.inherit.extend(
                                cl.objects.content)
                            classes[-1].functions.inherit.extend(
                                cl.functions.content)
                            classes[-1].specials.inherit.extend(
                                cl.specials.content)
                            break
        ...
        submodulename = scancodesignature(fl, "self.submodules += ",
                                              "",
                                              "self.submodules += ")
        ...
        submodulename = scancodesignature(fl, "setattr(self.submodules, ",
                                              "",
        ...
        clockdomainname = scancodesignature(fl, "self.clock_domains.",
                                                " = ClockDomain(",
                                                "self.clock_domains.")
        if len(classes) > 0 and "" != clockdomainname:
            classes[-1].clockdomains.content.append(clockdomainname)

        if -1 != fl.find("self.specials +=") or specialslineunfinished:
            if -1 != fl.find("self.specials += [") and -1 == fl.find("]"):
                specialslineunfinished = True
            if -1 == fl.find("self.specials += [") and -1 != fl.find("]"):
                specialslineunfinished = False
            instancestarts = fl.find("Instance(")
            if -1 != instancestarts:
                instancestarts = instancestarts + 10
                specialsname = fl[instancestarts:
                                  instancestarts +
                                  fl[instancestarts:].find("\"")]
                if len(classes) > 0 and "" != specialsname:
                    classes[-1].specials.content.append(specialsname)
    ...
    for cl in classes:
        ...
        clg.add(dwg.text(importedclasses,
                         insert=(-400, stl.idx*stl.height),
    ...
    for cl in classes:
        ...
        clg.add(dwg.text(".".join(cl.hierarchy) +
                         "." +
                         cl.name,
                         insert=(stl.column*stl.length*4 + 10,
                                 stl.idx*stl.height),
                         fill=svgwrite.rgb(0, 64, 64, 'RGB')))
        clg.add(dwg.line((stl.column*stl.length*4 + 0,
                          stl.idx*stl.height + 4),
                         (stl.column*stl.length*4 +
                          stl.length,
                          stl.idx*stl.height + 4),
        ...
        cl.submodules.svgdrawobject(stl, dwg, clg)
        cl.specials.svgdrawobject(stl, dwg, clg, True)
        clg.add(dwg.rect(insert=(stl.column*stl.length*4 + 0,
                                 stl.rectstartidx*stl.height),
                         size=(stl.length,
                               stl.height*(stl.idx -
                                           stl.rectstartidx + 1)),
                         fill="none",
                         stroke=svgwrite.rgb(0, 0, 0, 'RGB'),
                         stroke_width=1))
        if stl.idx*stl.height > 4096:
            stl.column = stl.column + 1
            stl.idx = 0
        dwg.add(clg)
    dwg.save()
